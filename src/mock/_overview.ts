...
export const _appRelated = ['Charging', 'Battery', 'Photovoltacs', 'Energy', 'Solar'].map(
  (name, index) => {
...
export const _appInvoices = [...Array(5)].map((_, index) => {
  const category = ['Fleet Management', 'Photovoltics', 'Battery', 'Charging Infrastucture'][index];

  const status = ['progress', 'incomplete', 'progress', 'paid'][index];
